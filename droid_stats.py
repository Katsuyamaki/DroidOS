...
def process_stats():
 ...
 # === THE FIX IS HERE ===
        # We add .drop_duplicates(subset=['asset']) to ignore any double-entries 
        # that might have accidentally crept into the CSV history.
        prev_snap = full_df[full_df['timestamp'] == last_ts].drop_duplicates(subset=['asset'])
        ...
        if len(times) > 0:
            ...
            # === OPTIONAL FIX FOR 24H STATS TOO ===
            snap_24h = full_df[full_df['timestamp'] == past_24h_ts].drop_duplicates(subset=['asset'])
