    ...
    private val commandReceiver = object : BroadcastReceiver() {        override fun onReceive(context: Context?, intent: Intent?) {
            ...
            if (action == ACTION_OPEN_DRAWER) { 
                ...
                if (autoAdjustMarginForIME) {
                    ...
                    val hasShowAboveDockExtra = intent?.hasExtra("SHOW_KB_ABOVE_DOCK") == true
                    val showAboveDock = intent?.getBooleanExtra("SHOW_KB_ABOVE_DOCK", true) ?: true
                    ...
                    queueWindowManagerCommand(
                        Intent()
                            ...
                            .putExtra("HAS_SHOW_KB_ABOVE_DOCK", hasShowAboveDockExtra)
                            .putExtra("SHOW_KB_ABOVE_DOCK", showAboveDock)
                    )
                }
...
// [NEW] Robust Move Logic: Finds original slot and forces move with retries
    ...
    private fun toggleDrawer() {
        if (isExpanded) {
            ...
            // Refresh task IDs for all queued apps on drawer open.
            // This catches stale IDs from apps opened/closed/moved outside DroidOS
            // (e.g. via Samsung DeX launcher, recent apps, or display changes).
            Thread {
                try {
                    val observed = getObservedVisibleComponents(currentDisplayId)
                    for (app in selectedAppsQueue.toList()) {
                        val basePkg = AppCompatibilityRegistry.normalizePackage(app.getBasePackage())
                    ...
                    android.util.Log.d("DROIDOS_DEX", "DRAWER_REFRESH error: ${e.message}")
                }
            }.start()
...
// Sorts queue: fullscreen first, tiled by layout position (top-left to bottom-right), minimized newest-to-oldest
    ...
    private fun minimizeToHiddenDisplayLocked(taskId: Int, packageName: String, className: String?, targetId: Int): Boolean {
        ...
        try {
            ...
            val taskScoped = shouldUseTaskScopedIdentityForClass(packageName, normalizedClass)
    ...
    private fun minimizeTaskForQueueEntry(app: MainActivity.AppInfo, reason: String): Int {
        ...
        // Check: is the app visible on the current display?
        // If not visible here, the task ID is likely from a different display.
        // Skip minimize to avoid cross-display am commands that cause crashes/lockouts.
        ...
        if (!visibleOnCurrent) {
            android.util.Log.d("DROIDOS_DEX", "minimizeTaskForQueueEntry SKIP_NOT_VISIBLE tid=$tid pkg=$basePkg display=$currentDisplayId — not on this display, skipping")
            return tid
        }
    ...
    // Serialize retile resize work to avoid out-of-order WM batchResize calls during rapid IME toggles.
    // Stale requests are dropped so only the newest bounds snapshot is applied.
...
// =================================================================================
    ...
    private fun applyImeVisibilityFromQueue(intent: Intent) {
        ...
        // - DockIME path: prefer explicit extras; otherwise refresh from SharedPrefs.
        val prevDockToolbarState = lastKnownDockImeToolbarVisible
        val prevShowAboveDockState = lastKnownShowKbAboveDock
        val prevManualSession = imeMarginManualSessionActive
        if (visible) {
            if (manualToggle) {
                imeMarginManualSessionActive = true
            } else {
                imeMarginManualSessionActive = false
                val dockPrefs = getSharedPreferences("DockIMEPrefs", Context.MODE_PRIVATE)
                lastKnownDockImeToolbarVisible = if (hasDockExtra) {
                    dockVisible
                } else {
                    dockPrefs.getBoolean("dock_ime_visible", false)
                }
                lastKnownShowKbAboveDock = if (hasShowAboveDockExtra) {
                    showAboveDock
                } else {
                    val os = orientSuffix()
            ...
            imeMarginManualSessionActive = false
            if (hasDockExtra) {
                lastKnownDockImeToolbarVisible = dockVisible
            }
        ...
        if (visible) {
            ...
            // Single-path show apply: always retile immediately in the same queue turn.
            // This removes delayed show-commit/verify split paths that can double-draw
            // bottom-row windows and intermittently leave partial blank space.
